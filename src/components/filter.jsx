  ...
  return (
    <div className="absolute lg:top-40 md:top-32 sm:top-24 top-16 lg:inset-x-16 md:inset-x-14 sm:inset-x-12 inset-x-8 border-4 border-white bg-slate-700 rounded-2xl">
      ...
      {genres.map((genre, index) => (
          <div key={genre.id} onClick={() => update(genre.id)} className={`lg:text-3xl md:text-2xl sm:text-xl text-lg font-anta lg:p-3 md:p-2 sm:p-1 p-0 justify-self-center border rounded-full w-full truncate text-center ${fgenre[genre.id] == true ? "border-purple-500 bg-white text-purple-500" : "border-white text-white"}`}>
            {genre.name}
          </div>
        )
      )}
